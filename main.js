...
function probability(list1, array2) {
    ...
    function countLetters(wordsArray) {
      ...
      for (const word of wordsArray) {
        for (let i = 0; i < word.length; i++) {
          const letter = word[i];
          if (letterCounts[i][letter]) {
            letterCounts[i][letter]++;
          } else {
            letterCounts[i][letter] = 1;
          }
        }
      }
    ...
    // Count letters in list1
    const letterCountsList1 = countLetters(list1);
  
    // Find the most repeated letter at each index in list1
    const mostRepeatedLetters = Object.keys(letterCountsList1).map(index => {
      const letterCounts = letterCountsList1[index];
    ...
    // Return the 5 most repeated letters as a single word
    concatenatedWord=mostRepeatedLetters.join('');
    const closestMatch = findClosestMatch(concatenatedWord, list1);
    return closestMatch; 
    
  }
  
  function findClosestMatch(targetWord, list1) {
      ...
      for (const word of list1) {
        const distance = levenshteinDistance(targetWord, word);
    ...
    function constraint(list1, exit, nonexit) {
      const list2 = [];
      const listinter1 = [];
      const listinter2 = [];
      ...
      for (const i of list1) {
        ...
        if (count1 === exit.length) {
          listinter1.push(i);
        }
    
        for (const k of nonexit) {
          if (!i.includes(k)) {
            count2++;
          }
        }
    
        if (count2 === nonexit.length) {
          listinter2.push(i);
        }
      }
    
      const result = listinter1.filter(word => listinter2.includes(word));
      return result;
    }

  function position(list2, posdict1, posdict2, posdict3, green_letters, uncolored_set) {
    const list4 = [];
    for (const word of list2) {
      let flag = true;
      for (const i in posdict1) {
        if (posdict1[i] !== '') {
          if (posdict1[i] !== word[i]) {
            flag = false;
            break;
          }
        }
    ...
    const list5 = [];
    for (const word of list4) {
      let flag = true;
      for (const i in posdict2) {
        if (posdict2[i] !== '') {
          if (posdict2[i] === word[i]) {
            flag = false;
            break;
          }
        }
    ...
    const list6 = [];
    for (const word of list5) {
      let flag = true;
      for (const i in posdict3) {
        if (posdict3[i] !== '') {
          if (green_letters.has(posdict3[i])) {
            if (posdict3[i] === word[i]) {
              flag = false;
              break;
            }
          }
    ...
    const list7 = [];
    for (const word of list6) {
      if (uncolored_set.length && green_letters.size) {
        let flag = true;
        for (const i of uncolored_set) {
          if (green_letters.has(i)) {
            for (let k = 0; k < 5; k++) {
              if (posdict1[k] === '') {
                if (word[k] === i) {
                  flag = false;
  ...
  document.addEventListener("DOMContentLoaded", () => 
{
  ...
  function retrieveData() {
    ...
    let chosen = wordData.word;

let yellowLetters = new Set(wordData.yellowLetters.map(letterObj => letterObj.letter));
let greenLetters = new Set(wordData.greenLetters.map(letterObj => letterObj.letter));
let uncoloredSet = new Set(wordData.uncoloredLetters.map(letterObj => letterObj.letter));

let exit = [...new Set([...greenLetters, ...yellowLetters])];
let distinctLetters = new Set(exit);
let nonexit = [...uncoloredSet].filter(letter => !distinctLetters.has(letter));

let greenLettersObj = {};
wordData.greenLetters.forEach(letterObj => {
  let index = letterObj.index;
  let letter = letterObj.letter;
  greenLettersObj[index] = letter;
});
for (let i = 0; i < 5; i++) {
  if (!greenLettersObj.hasOwnProperty(i)) {
    greenLettersObj[i] = '';
  }
}
let posdict1 = Object.fromEntries(Object.entries(greenLettersObj).sort((a, b) => a[0] - b[0]));

let yellowLettersObj = {};
wordData.yellowLetters.forEach(letterObj => {
  let index = letterObj.index;
  let letter = letterObj.letter;
  yellowLettersObj[index] = letter;
});
for (let i = 0; i < 5; i++) {
  if (!yellowLettersObj.hasOwnProperty(i)) {
    yellowLettersObj[i] = '';
  }
}
let posdict2 = Object.fromEntries(Object.entries(yellowLettersObj).sort((a, b) => a[0] - b[0]));

let uncoloredLettersObj = {};
wordData.uncoloredLetters.forEach(letterObj => {
  let index = letterObj.index;
  let letter = letterObj.letter;
  uncoloredLettersObj[index] = letter;
});
for (let i = 0; i < 5; i++) {
  if (!uncoloredLettersObj.hasOwnProperty(i)) {
    uncoloredLettersObj[i] = '';
  }
}
let posdict3 = Object.fromEntries(Object.entries(uncoloredLettersObj).sort((a, b) => a[0] - b[0]));

// console.log(chosen);
// console.log(exit);
// console.log(nonexit);
// console.log(greenLetters);
// console.log(uncoloredSet);
// console.log(posdict1);
// console.log(posdict2);
// console.log(posdict3);
...
let list2 = constraint(list1, exit, nonexit);
let list3 = position(list2, posdict1, posdict2, posdict3,greenLetters, uncoloredSet);
list1 = list3;

// Print the variables if needed
let predicted_word = probability(list3);
predicted_word = predicted_word.slice(0, 5);
console.log("Predicted word:", predicted_word);


    const predictedWord = predicted_word;
    words.push(predictedWord);
    if (predictedWord === "") {
        alert("Sorry, cannot find the word. Try again.");
        resetGame();
        return;
        }      
    displayPredictedWord(predictedWord); 
    console.log("pred:",predictedWord)
    currentWordIndex++
